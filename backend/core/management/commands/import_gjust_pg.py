...
EXCEL_PATH = Path(__file__).resolve().parents[4] / 'Data ' / 'GJUST_PG_Course_Eligibility.xlsx'
...
def parse_seats(val):
    if val is None or val == 'NULL':
        return None
    s = str(val).strip()
    if s.lower() in ('zero session', 'zero', ''):
        return 0
    if '+' in s:
        try:
...
class Command(BaseCommand):
    help = 'Import GJUS&T PG courses from GJUST_PG_Course_Eligibility.xlsx'
        ...
        for cd in courses_data:
            ...
            for cc in cc_list:
                ...
                EligibilityCriteria.objects.update_or_create(
                    ...
                    defaults={
                        'min_graduation_percentage': min_pct,
                        'required_graduation': req_subj,
