...
EXCEL_PATH = Path(__file__).resolve().parents[4] / 'Data ' / 'MDU_PG_Course_Eligibility.xlsx'
...
class Command(BaseCommand):
    help = 'Import MDU PG courses from MDU_PG_Course_Eligibility.xlsx'
    ...
    def handle(self, *args, **options):
        ...
        mdu, created = University.objects.update_or_create(
            short_name='MDU',
            defaults={
                'name': uni_data['name'],
        ...
        self.stdout.write(f'{"Created" if created else "Updated"} university: {mdu}')
            ...
            for cc in cc_list:
                ...
                AdmissionCycle.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        'application_link': 'https://mduadmission.samarth.edu.in/index.php/site/login',
                        'status': status,
                        'notes': f'Admission mode: {cycle.get("admission_mode", "")}',
                    }
                )

                self.stdout.write(f'  {course.name} ({stream}) — seats:{seats}, fee:{fee}')

        self.stdout.write(self.style.SUCCESS(
            f'\nMDU PG import complete!\n'
