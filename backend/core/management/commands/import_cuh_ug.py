...
EXCEL_PATH = Path(__file__).resolve().parents[4] / 'Data ' / 'CUH_UG_Course_Eligibility.xlsx'
...
class Command(BaseCommand):
    help = 'Import CUH UG courses from CUH_UG_Course_Eligibility.xlsx'
    ...
    def handle(self, *args, **options):
        ...
        cuh, created = University.objects.update_or_create(
            short_name='CUH',
            defaults={
                'name': uni_data['name'],
        ...
        self.stdout.write(f'{"Created" if created else "Updated"} university: {cuh}')
